...
struct QuizManager {
            ...
            let task = session.dataTask(with: url) { data, response, error in
                ...
                if let safeData = data{
                    ...
                    if let quiz = self.parseJSON(quizdata: safeData){
                        delegate?.didUpdateQuiz(quiz:quiz)
                    }
    ...
    func returnProgress() -> Float{
        let progress = Float(index + 1) / Float(maxQuestion)
        return progress
    }
    
    func delateTrash1( quest : String ) -> String{
        var newQuest = quest
        newQuest =  quest.replacingOccurrences(of: "&#039;", with: "'")
        
       return newQuest
    }
    func delateTrash2( quest : String ) -> String{
        var newQuest = quest
        newQuest =  quest.replacingOccurrences(of: "&quot;", with: "''")
       return newQuest
    }
